...
module.exports = class WechatClient extends WechatBase {
  ...
  _searchContactId(username){
    var _index;
    _.each(this.contactList, (user, index) => {
      if (user.RemarkName.match(username) ||
        user.NickName.match(username)) {
          _index = index;
      }
    });
    return _index;
  ...
  _printMsg(msg) {
    ...
    var username = this._getUserNickName(msg.FromUserName);
    var _username = this._getRealUserName(username)
    var index = this._searchContactId(_username);
    var from_user = "#" + String(index) + " - " + username;

    var time = new Date(msg.CreateTime * 1000).toLocaleTimeString();
    var data = {
      time: time,
      from: this._getUserNickName(msg.FromUserName),
      index: this.getContactId(this._getUserNickName(msg.FromUserName)),
      message: msg.Content,
    };
    ...
    if (msg.ActualSender) {
      ...
      data.index =  this.getRoomId(this._getUserNickName(msg.ActualSender));
    }
